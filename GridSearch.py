...
class Models:
    ...
    def model_fit(self, X_train, y_train):
        if self.model_name.startswith('linear'):
            ...
            self.model = MLPRegressor(
                hidden_layer_sizes=(35, 10), max_iter=100000,
                solver='lbfgs', 
                random_state=self.seed, 
                learning_rate_init=0.001, 
                momentum=0.9,
...
# 打印最优模型的参数
model = joblib.load("models/ours_log_grid_mlp_1.pkl")
score = -model.best_score_
best_model = model.best_estimator_
print("Best hyper parameters", best_model.get_params()["hidden_layer_sizes"])
